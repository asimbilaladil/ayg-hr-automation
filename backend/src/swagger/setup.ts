      ...
      Candidate: {
        ...
        properties: {
          ...
          status: {
            type: 'string',
            enum: [
              'pending', 'reviewed', 'called', 'not_found', 'processed_no_resume',
              'voicemail', 'outbound_call', 'inbound_call_received', 'interview-booked', 'cancelled',
            ],
          },
          ...
          interviewAnswers: { type: 'string', nullable: true, description: 'Stringified JSON array of [{category, question, answer}]' },
          endedReason: { type: 'string', nullable: true, description: 'VAPI ended-reason: voicemail, machine_end_beep, hangup, silence-timed-out, etc.' },
          nextCallAt: { type: 'string', format: 'date-time', nullable: true, description: 'Cooldown expiry — null means call anytime; auto-set to now+1h on voicemail' },
          lastCallAt: { type: 'string', format: 'date-time', nullable: true, description: 'Timestamp of the most recent call attempt' },
        ...
        parameters: [
          {
            ...
            schema: {
              type: 'string',
              enum: ['pending', 'reviewed', 'called', 'not_found', 'processed_no_resume', 'voicemail', 'outbound_call', 'inbound_call_received', 'interview-booked', 'cancelled'],
            },
          },
          { name: 'location', in: 'query', schema: { type: 'string' } },
          { name: 'postingName', in: 'query', schema: { type: 'string' } },
          { name: 'aiRecommendation', in: 'query', schema: { type: 'string', enum: ['HIRE', 'MAYBE', 'REJECT'] } },
          ...
          { name: 'nextCallAtBefore', in: 'query', schema: { type: 'string', format: 'date-time' }, description: 'Return only candidates where nextCallAt IS NULL or nextCallAt <= value (ISO datetime). Used by outbound workflow to skip voicemail cooldowns.' },
        ],
        responses: {
            ...
            'application/json': {
              schema: {
                ...
                properties: {
                  ...
                  aiRecommendation: { type: 'string', enum: ['HIRE', 'MAYBE', 'REJECT'] },
            ...
            'application/json': {
              schema: {
                ...
                properties: {
                  ...
                  status: {
                    type: 'string',
                    enum: ['voicemail', 'outbound_call', 'inbound_call_received', 'interview-booked', 'cancelled', 'called', 'no-answer'],
                    description: 'n8n/Vapi call outcome. "voicemail" auto-sets a 1-hour nextCallAt cooldown.',
                  },
                  recordingUrl: { type: 'string', description: 'URL to the VAPI call recording' },
                  interviewAnswers: { type: 'string', description: 'Stringified JSON array of [{category, question, answer}]' },
                  nextCallAt: { type: 'string', format: 'date-time', description: 'ISO datetime — when cooldown expires. Auto-set to now+1h when status=voicemail if omitted.' },
                  endedReason: { type: 'string', description: 'VAPI ended-reason: voicemail, machine_end_beep, machine_end_silence, machine_end_other, hangup, silence-timed-out, etc.' },
                },
              },
            },
          },
